...
public class CustomerHandler {
	...
	public BusinessPartnerService bupaService = new DefaultBusinessPartnerService();
	private static final String S4HANA_DEST_NAME = "S4HANA_basic";
	...
	public QueryResponse queryA_Customer(QueryRequest qryRequest) {
		...
		try {
			List<BusinessPartner> businessPartnerList = bupaService.getAllBusinessPartner()
	...
	public ReadResponse readA_Customer(ReadRequest readRequest) {
		...
		if (keys != null && keys.size() == 1) {
			...
			try {
				final BusinessPartner businessPartnerEntity = bupaService
	...
	public CreateResponse writeA_Customer(CreateRequest createRequest) {
		...
		try {
			...
			BusinessPartner businessPartner = bupaService.createBusinessPartner(bpInput)
					.execute(endpoint);
	...
	public void setBupaService(BusinessPartnerService service) {
		this.bupaService = service;
	}
}
